...
13. Public invalidation dispatcher configuration boundary: `Settings.public_invalidation_dispatcher` currently supports `none`, the contract-only adapter value `contract`, and the Cloudflare config-only adapter value `cloudflare`; these can be supplied through `NAIRI_PUBLIC_INVALIDATION_DISPATCHER=none|contract|cloudflare`, while unsupported values remain rejected before any external dispatcher behavior can be implied.
14. Public invalidation dispatcher adapter boundary: the app builds a configured `PublicInvalidationDispatcher`; `none` returns `dispatch_skipped`, `no_dispatcher_configured`, `attempted=false`, and `attemptedAt=null`; `contract` returns deterministic contract-only bookkeeping with `dispatch_skipped`, `contract_only_adapter`, `attempted=true`, and `attemptedAt=publishedAt`; `cloudflare` returns config-only disabled bookkeeping with `dispatch_skipped`, `cloudflare_adapter_disabled`, `attempted=false`, and `attemptedAt=null` without contacting Cloudflare or any external system.
...
18. Public invalidation execution boundary: recorded execution, dispatch bookkeeping, dispatcher configuration, the no-op dispatcher interface, the contract-only dispatcher adapter, the Cloudflare config-only disabled adapter, route-level dispatcher result mapping, dispatcher result persistence, and dispatcher exception bookkeeping are durable/configuration/in-process bookkeeping only; they do not trigger CDN purge, Cloudflare API calls, `revalidateTag`, `revalidatePath`, webhooks, cache headers, or any external invalidation side effect.
