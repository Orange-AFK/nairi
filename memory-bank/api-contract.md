...
14. Public invalidation dispatcher adapter boundary: the app builds a configured `PublicInvalidationDispatcher`; `none` returns `dispatch_skipped`, `no_dispatcher_configured`, `attempted=false`, and `attemptedAt=null`; `contract` returns deterministic contract-only bookkeeping with `dispatch_skipped`, `contract_only_adapter`, `attempted=true`, and `attemptedAt=publishedAt`; `cloudflare` returns config-only disabled or missing-settings bookkeeping with `dispatch_skipped`, `cloudflare_adapter_disabled` or `cloudflare_adapter_missing_settings`, `attempted=false`, and `attemptedAt=null` without contacting Cloudflare or any external system. The Cloudflare adapter can build an inert `CloudflarePurgeRequestPlan` with `method=POST`, path `/client/v4/zones/{zone_id}/purge_cache`, and a deduplicated ordered `files` body from public invalidation surfaces; the plan contains no token or Authorization data and is not executed by dispatch.
