...
1. Status: completed, merged, and read back on `main`.
2. Completed scope: non-object `Draft metadata JSON` now renders targeted safe error copy while keeping the invalid metadata branch side-effect-free before `updatePost`.
