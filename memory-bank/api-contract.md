...
14. Public invalidation dispatcher interface boundary: the app builds a configured `PublicInvalidationDispatcher` and currently only provides a no-op dispatcher for `none`; the no-op dispatcher returns `dispatch_skipped`, `no_dispatcher_configured`, `attempted=false`, and `attemptedAt=null` without contacting any external system.
...
18. Public invalidation execution boundary: recorded execution, dispatch bookkeeping, dispatcher configuration, the no-op dispatcher interface, route-level dispatcher result mapping, dispatcher result persistence, and dispatcher exception bookkeeping are durable/configuration/in-process bookkeeping only; they do not trigger CDN purge, `revalidateTag`, `revalidatePath`, webhooks, cache headers, or any external invalidation side effect.
