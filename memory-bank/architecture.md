...
15. The publish API returns that invalidation record with `mode=recorded` plus dispatch bookkeeping. API settings currently expose `public_invalidation_dispatcher=none|contract`, with env wiring for `NAIRI_PUBLIC_INVALIDATION_DISPATCHER=none|contract`; unsupported dispatcher values are still rejected before external dispatch behavior exists.
16. App creation builds `app.state.public_invalidation_dispatcher` from settings. Current implementations are `NoopPublicInvalidationDispatcher` for `none` and `ContractPublicInvalidationDispatcher` for `contract`; both return deterministic bookkeeping and do not call external systems.
