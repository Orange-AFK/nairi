...
1. Status: runtime API client boundary completed through Admin Publish Request Persistence Boundary; Publish Request Resolve Workflow Boundary is completed, merged, and read back.
...
5. Current detailed admin status is tracked in `Admin Runtime API Client Boundary`; next admin candidate is a separate published-history/list module if product wants it, or another narrow admin edit boundary.
...
1. Status: completed and merged through Publish Request Resolve Workflow Boundary.
...
3. Next candidate: a separate published-history/list module if product wants it, or another narrow admin edit boundary.
