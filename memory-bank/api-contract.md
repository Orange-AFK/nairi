...
1. Method: `GET`
2. Path: `/api/v1/categories`
3. Scope: `taxonomy:read`
4. Request body fields: none
5. Response fields: `items`
6. Item fields: `categoryId`, `name`, `slug`, `description`, `createdAt`, `updatedAt`
7. List items are ordered by `name` ascending.
8. Audit event: none for list readback.
9. Clients: admin console for category selection and management.
...
5. Response fields: `categoryId`, `name`, `slug`, `description`, `createdAt`, `updatedAt`
...
10. `categoryId` is deterministic: `cat-{slug}`.

### Get Category

1. Method: `GET`
2. Path: `/api/v1/categories/{category_id}`
3. Scope: `taxonomy:read`
4. Request body fields: none
5. Response fields: `categoryId`, `name`, `slug`, `description`, `createdAt`, `updatedAt`
6. Errors: `404` with code `not_found` when `category_id` is unknown.
...
3. Scope: `taxonomy:write`
4. Request body fields: `name`, `slug`, `description`
5. Response fields: `categoryId`, `name`, `slug`, `description`, `updatedAt`
...
8. Errors: `404` with code `not_found` when `category_id` is unknown.
9. Errors: `409` with code `conflict` when the requested `slug` already belongs to another category.
...
12. Public invalidation dispatch boundary: the API records `dispatch.status=dispatch_skipped`, `dispatch.reason=no_dispatcher_configured`, `dispatch.attempted=false`, and `dispatch.attemptedAt=null` on the durable publish job as future-safe dispatch semantics when no dispatcher is configured.
13. Public invalidation dispatcher configuration boundary: `Settings.public_invalidation_dispatcher` currently supports `none`, the contract-only adapter value `contract`, and the Cloudflare config-only adapter value `cloudflare`; these can be supplied through `NAIRI_PUBLIC_INVALIDATION_DISPATCHER=none|contract|cloudflare`, while unsupported values remain rejected before any external dispatcher behavior can be implied. Cloudflare adapter configuration currently also accepts optional `NAIRI_PUBLIC_INVALIDATION_CLOUDFLARE_ZONE_ID` and `NAIRI_PUBLIC_INVALIDATION_CLOUDFLARE_API_TOKEN`; the token is treated as a secret value and must not be logged or serialized in plain text.
14. Public invalidation dispatcher adapter boundary: the app builds a configured `PublicInvalidationDispatcher`; `none` returns `dispatch_skipped`, `no_dispatcher_configured`, `attempted=false`, and `attemptedAt=null`; `contract` returns deterministic contract-only bookkeeping with `dispatch_skipped`, `contract_only_adapter`, `attempted=true`, and `attemptedAt=publishedAt`; `cloudflare` returns missing-settings bookkeeping with `dispatch_skipped`, `cloudflare_adapter_missing_settings`, `attempted=false`, and `attemptedAt=null` until both Cloudflare settings are configured. When Cloudflare settings are complete, dispatch performs only an in-process dry-run request-plan build and returns `dispatch_skipped`, `cloudflare_adapter_dry_run`, `attempted=true`, and `attemptedAt=publishedAt` without contacting Cloudflare or any external system. The Cloudflare adapter can build an inert `CloudflarePurgeRequestPlan` with `method=POST`, path `/client/v4/zones/{zone_id}/purge_cache`, and a deduplicated ordered `files` body from public invalidation surfaces; the plan contains no token or Authorization data and is not exposed in dispatch results.
15. Public invalidation dispatcher route integration boundary: after a successful publish storage transition, the publish route calls `app.state.public_invalidation_dispatcher.dispatch()` with the recorded public surfaces and `publishedAt`, then builds the response `publicInvalidation.dispatch` object from that dispatcher result.
16. Public invalidation dispatcher persistence boundary: after the dispatcher returns, the publish route records the dispatcher result back onto the durable `publish_jobs` row, so response `publicInvalidation.dispatch` and stored dispatch fields use the same status, reason, attempted flag, and attempted timestamp.
17. Public invalidation dispatcher error policy boundary: if the configured dispatcher raises, the publish route keeps the already-successful publish transition, records `dispatch.status=dispatch_failed`, `dispatch.reason=dispatcher_exception`, `dispatch.attempted=true`, and `dispatch.attemptedAt=publishedAt` in both the response and durable `publish_jobs` row. Dispatch persistence itself fails closed when the target publish job row is missing.
