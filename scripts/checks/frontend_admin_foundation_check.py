...
required_app = [
    ...
    'aria-label={postListLabel === "Drafts" ? "Draft posts" : "Content items"}',
    ...
    "Publish confirmed draft",
    "Draft published at",
    "Draft could not be published.",
    "Published detail is read-only in the draft review workflow.",
    "Draft editing and publishing controls are hidden for this content item.",
    "Draft changes saved.",
    "Draft changes could not be saved.",
    "Reload the draft detail, review the latest revision, and retry.",
...
required_test = [
    ...
    "renders non-draft detail as read-only without draft workflow controls",
    ...
    "persists a publish review request through the injected request contract without publishing",
...
required_client = [
    ...
    "/api/v1/posts/${encodeURIComponent(postId)}/publish-requests",
    'method: "POST"',
...
required_client_test = [
    ...
    "creates a publish review request through the authenticated management API without publishing",
