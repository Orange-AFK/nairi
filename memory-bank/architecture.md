...
15. The publish API returns that invalidation record with `mode=recorded` plus dispatch bookkeeping. API settings currently expose `public_invalidation_dispatcher=none|contract|cloudflare`, with env wiring for `NAIRI_PUBLIC_INVALIDATION_DISPATCHER=none|contract|cloudflare`; unsupported dispatcher values are still rejected before external dispatch behavior exists.
16. App creation builds `app.state.public_invalidation_dispatcher` from settings. Current implementations are `NoopPublicInvalidationDispatcher` for `none`, `ContractPublicInvalidationDispatcher` for `contract`, and `CloudflarePublicInvalidationDispatcher` for `cloudflare`; all return deterministic bookkeeping and do not call external systems. The Cloudflare adapter is config-only and disabled, returning `cloudflare_adapter_disabled` without Cloudflare API calls.
