...
16. App creation builds `app.state.public_invalidation_dispatcher` from settings. Current implementations are `NoopPublicInvalidationDispatcher` for `none`, `ContractPublicInvalidationDispatcher` for `contract`, and `CloudflarePublicInvalidationDispatcher` for `cloudflare`; all return deterministic bookkeeping and do not call external systems. The Cloudflare adapter is config-only and disabled, returning `cloudflare_adapter_missing_settings` until both Cloudflare settings are configured and `cloudflare_adapter_disabled` once settings are present. It can build an inert `CloudflarePurgeRequestPlan` for `/client/v4/zones/{zone_id}/purge_cache` from deduplicated ordered surfaces, but dispatch does not execute the plan or call Cloudflare.
