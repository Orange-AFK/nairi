...
17. Public invalidation execution boundary: recorded execution, dispatch bookkeeping, dispatcher configuration, the no-op dispatcher interface, route-level dispatcher result mapping, and dispatcher result persistence are durable/configuration/in-process bookkeeping only; they do not trigger CDN purge, `revalidateTag`, `revalidatePath`, webhooks, cache headers, or any external invalidation side effect.
17. Duplicate capability warning: admin, MCP, and agents must use this capability instead of creating parallel publish endpoints.
