...
16. The publish flow does not trigger CDN purge, Next.js tag/path revalidation, webhooks, cache headers, or any external invalidation side effect.
17. Admin, MCP, and authorized clients can read published summaries through `GET /api/v1/posts?status=published` and published detail through `GET /api/v1/posts/{post_id}` with `posts:read` scope in the current scaffold.
18. Public clients read published summaries through `GET /api/v1/public/posts`, which is anonymous and returns only public-safe fields.
19. Public clients read published detail through `GET /api/v1/public/posts/{slug}`, which is anonymous, slug-based, published-only, and returns a public-safe detail response.
20. Public detail includes both authored `content` and a minimal sanitized `bodyHtml` render output. The renderer is intentionally not full MDX execution; it only handles a small Markdown subset and strips script blocks from rendered HTML.
21. Public clients must not reuse authenticated content-management routes.
22. Authenticated published summary lists can currently be filtered by tag membership, category id, or series id.
23. Authenticated and public published summary lists can currently be paginated with `limit` and an item-id `cursor`.
24. Public filtering inputs, full MDX/component rendering, scheduling semantics, external invalidation execution, and the job runner remain future work under documented state rules.
